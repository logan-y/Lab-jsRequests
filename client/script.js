...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/strange`)
    .then(response => {
...
const queryFn = () => {
       axios.get(`http://localhost:3000/query-test?num=false`)
       .then(response => {
    ...
    document.querySelector('#query-button').addEventListener('click', queryFn)
...
const createFood = event => {
    ...
    .then(response => {
        foodStr = document.createElement('p');
            foodStr.textContent = response.data;
        console.log('response', response.data)
        let foodText = document.querySelector('#food-text');
        foodText.style.display = 'block';
        foodText.style.backgroundColor = 'green';
        foodText.textContent = response.data;
    })
    .catch(error => {
...
////////////////
    ...
    /* 
       ...
       1: Send no queries on the URL -- what happened? 
            ...
            the index file returns response string that says "you sent an empty query!"
       
       2: Send more than 1 query on the URL -- what happened? 
       
            console.log in the index.js will show "you sent more than 1 query!" - followed by a list of the objects.
       */
